...
///
/// This is the "over-the-wire CLAUDE.md" — it tells agents what Biblion
/// provides and how to use it, without requiring them to read any docs.
const SERVER_INSTRUCTIONS: &str = "\
Biblion gives you access to a Zotero reference library. Use citation keys (citekeys) as identifiers.

## What you can do

**Search & browse**: `zotero_search` (by title, DOI, abstract), `zotero_get_item` (full metadata by citekey), \
`zotero_get_recent`, `zotero_get_collections`, `zotero_get_collection_items`.

**Cite & export**: `zotero_get_bibtex` (BibTeX/BibLaTeX), `zotero_get_bibliography` (formatted APA/IEEE), \
`zotero_export_bibtex` (export a collection).

**PDF & attachments**: `zotero_get_pdf_path` (filesystem path + MD5 hash), `zotero_list_attachments`, \
`paper_resolve_pdf` (find open-access PDF by DOI/title from 9 academic sources).

**Write** (if enabled): `zotero_create_item`, `zotero_update_item`, `zotero_add_tags`, `zotero_add_note`, \
`zotero_create_collection`, `zotero_add_to_collection`, `zotero_delete_item`, `zotero_merge_items`, \
`zotero_attach_pdf`, `zotero_fetch_missing_pdfs`.

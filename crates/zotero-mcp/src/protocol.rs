...
mod tests {
    ...
    fn error_response_serializes() {
        let resp = JsonRpcResponse::error(Some(json!(1)), -32600, "Invalid Request".into());
        let json = serde_json::to_value(&resp).unwrap();
    ...
    fn tool_call_params_deserializes() {
        let raw = r#"{"name":"zotero_search","arguments":{"query":"test"}}"#;
        let params: ToolCallParams = serde_json::from_str(raw).unwrap();
        assert_eq!(params.name, "zotero_search");
    ...
    #[test]
    fn tool_call_result_error_serializes_is_error() {
